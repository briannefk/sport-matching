...
// 00:00 到 24:00，讓建立者可以自由選擇開始與結束時間
const TIME_OPTIONS = Array.from({ length: 25 }, (_, hour) => {
...
function canCancelBeforeStart(event) {
  ...
  const startTime = new Date(
    `${event.date}T${String(startHour).padStart(2, "0")}:${String(startMinute).padStart(2, "0")}:00`
  );
...
export default function App() {
  ...
  async function loadEvents() {
    ...
    setSelectedId((previous) => {
      if (previous && arrangedEvents.some((event) => event.id === previous)) {
        return previous;
      }
  ...
  const myMembership = selectedEvent?.members.find((member) => {
  ...
  function updateStartTime(startTime) {
    ...
    const nextEndTime = validEndTimes.includes(currentEndTime)
      ? currentEndTime
      : validEndTimes[0];
        ...
        <main className="grid">
              ...
              <div className="time-row">
                ...
                <Field label="結束時間">
                  <select value={eventForm.endTime} onChange={(e) => updateEndTime(e.target.value)}>
                    {getValidEndTimes(eventForm.startTime).map((time) => (
                      <option key={time}>{time}</option>
                    ))}
                  </select>
                </Field>
          ...
          <section className="panel events">
            <div className="section-head">
              <h2>可加入的活動</h2>
              <button onClick={loadEvents}>重新整理</button>
            </div>

            {loading ? (
              <div className="empty">讀取活動中...</div>
            ) : events.length === 0 ? (
              <div className="empty">目前沒有活動，請建立第一場活動。</div>
            ) : (
              <div className="cards">
                {events.map((event) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="card-footer">
                        <span>{event.members.length} / {event.maxPlayers} 人</span>
                        <b>
                          {event.court
                            ? `${event.court} 號場`
                            : `還差 ${Math.max(event.maxPlayers - event.members.length, 0)} 人`}
                        </b>
                      </div>
                    </button>
                  );
          ...
          <section className="panel detail">
            <div className="detail-head">
              <div>
                ...
                <h2>
                  {getSport(selectedEvent.sport).icon} {selectedEvent.title || `${selectedEvent.sport}活動`}{" "}
                  {selectedEvent.isPrivate && <small className="private-title">🔒 私人</small>}
                </h2>

                <span>
                  {selectedEvent.sport}　｜　{selectedEvent.date}　{selectedEvent.timeSlot}
                  {selectedEvent.court && `　｜　${selectedEvent.court} 號場`}
                </span>

                {ownerInviteCode && (
                  <div className="invite-display">
                    邀請碼：<b>{ownerInviteCode}</b>
                    <small>
            ...
            {selectedEvent.status === "open" && (
              <form className="join" onSubmit={joinSelectedEvent}>
                ...
                <Field label="位置 / 角色">
                  ...
                  >
                    {getSport(selectedEvent.sport).roles.map((role) => (
                      <option key={role}>{role}</option>
                    ))}
                  </select>
                </Field>

                <Field label={selectedEvent.isPrivate ? "邀請碼（必填）" : "邀請碼（選填）"}>
                  <input
                    maxLength="6"
                    placeholder={selectedEvent.isPrivate ? "輸入 6 碼" : "公開活動可不填"}
            ...
            {myMembership && (
              <div className="cancel-box">
                <div>
                  ...
                  <small>
                    {canCancelBeforeStart(selectedEvent)
                      ? "活動開始前 30 分鐘以前可以取消報名"
                      : "已超過可取消時間"}
                  </small>
                </div>
                ...
                <button
                  ...
                  disabled={!canCancelBeforeStart(selectedEvent)}
                >
                  取消報名
                </button>
              </div>
            ...
            <div className="members">
              <h3>
                參加名單 <small>{selectedEvent.members.length} / {selectedEvent.maxPlayers}</small>
              </h3>
            ...
            {selectedEvent.status === "full" && (
              <div className="attendance">
                <div className="section-head">
                  <h3>到場簽到</h3>
                  <span>
                    {selectedEvent.members.filter((m) => m.attendance_status === "checked_in").length} /{" "}
                    {selectedEvent.members.length} 人已到場
                  </span>
                </div>

                {!session ? (
                  <div className="empty">請先登入後進行本人簽到。</div>
                ) : !myMembership ? (
                  <div className="empty">你未加入本活動，無法進行簽到。</div>
                ) : myMembership.attendance_status === "checked_in" ? (
                  <div className="success">✓ 你已完成簽到</div>
                ) : (
            ...
            {selectedTeams && (
              <div className="match">
                ...
                <div className="versus">
                  <Team name="A 隊" players={selectedTeams.teamA} />
                  <b className="vs">VS</b>
                  <Team name="B 隊" players={selectedTeams.teamB} />
                </div>
              </div>
            )}
          </section>
...
.balls span {
  ...
  padding: 18px;
  border-radius: 27px;
  box-shadow: 0 10px 25px #0f2f5b18;
}
...
.join {
  ...
  border-radius: 16px;
  display: grid;
  grid-template-columns: 115px repeat(3, minmax(130px, 1fr)) 125px;
  gap: 13px;
  align-items: end;
}
